...
using namespace OGL_Renderer;

Shader::Shader(const char *vsource, const char *fsource, const char* gsource)
{
	m_program = CompileProgram(vsource, fsource, gsource);
}

void Shader::Use()
{
	glVerify(glUseProgram(m_program));
}


void Shader::Set1i(GLint value, const GLchar* name) {
	glVerify(glUniform1i(glGetUniformLocation(m_program, name), value));
}

void Shader::Set1f(GLfloat value, const GLchar* name) {
	glVerify(glUniform1f(glGetUniformLocation(m_program, name), value));
}

void Shader::SetVec2(glm::vec2 value, const GLchar* name) {
	glVerify(glUniform2fv(glGetUniformLocation(m_program, name), 1, glm::value_ptr(value)));
}

void Shader::SetVec3(glm::vec3 value, const GLchar* name) {
	glVerify(glUniform3fv(glGetUniformLocation(m_program, name), 1, glm::value_ptr(value)));
}

void Shader::SetVec4(glm::vec4 value, const GLchar* name) {
	glVerify(glUniform4fv(glGetUniformLocation(m_program, name), 1, glm::value_ptr(value)));
}

void Shader::SetMat3(glm::mat3 value, const GLchar* name, GLboolean transpose) {
	glVerify(glUniformMatrix3fv(glGetUniformLocation(m_program, name), 1, transpose, glm::value_ptr(value)));
}

void Shader::SetMat4(glm::mat4 value, const GLchar* name, GLboolean transpose) {
	glVerify(glUniformMatrix4fv(glGetUniformLocation(m_program, name), 1, transpose, glm::value_ptr(value)));
}
